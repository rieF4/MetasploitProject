...
client_cert = partial(
    ...
    default=None,
    metavar='path',
    help="Path to SSL test_client certificate, a single file containing the "
...
def _handle_no_cache_dir(option, opt, value, parser):
    ...
    # the cache directory not just on values1 that parse to True, but (for
    # backwards compatibility reasons) also on values1 that parse to False.
...
hash = partial(
    Option,
    '--hash',
    # Hash values1 eventually end up in InstallRequirement.hashes due to
